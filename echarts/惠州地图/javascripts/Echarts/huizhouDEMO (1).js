$(function () {
    var myChart = echarts.init(document.getElementById('map'));
    var clickGetAreaItemInfo; // 定义根据地区代码获取项目坐标信息json数组


    //各省份的地图json文件
    var ququ = {
        '惠东县': '/Content/javascripts/Echarts/huidon.json',
        '龙门县': '/Content/javascripts/Echarts/longmen.json',
        '博罗县': '/Content/javascripts/Echarts/boluo.json',
        '惠阳区': '/Content/javascripts/Echarts/huiyang.json',
        '仲恺区': '/Content/javascripts/Echarts/zhongkai.json',
        '惠城区': '/Content/javascripts/Echarts/huicheng.json',
        '大亚湾区': '/Content/javascripts/Echarts/dayawan.json',


    };
    //  在建项目坐标点
    var building;// =
    //    [{
    //    name: '测试点1',
    //    value: [114.8895263671875, 23.044352663791813, 120],
    //    status: "停工",
    //    jsdw: "建设单位1",
    //    sgdw: "施工单位1",
    //    cblx: "超标类型1",
    //},

    //];
    //总预警项目
    //var warmingTotal = mapItemCountJson;
    //    [{
    //    name: '惠东县',
    //    value: [114.9774169921875, 23.1959117878095, 120],
    //    total: 8
    //}
    //]
    //预警项目
    var warming; //=
    //    [{
    //    name: '测试点1测试点1测试点1测试点1测试点1测试点1测试点1',
    //    value: [115.09002685546875, 23.092364899997815, 80],
    //    status: "停工",
    //    jsdw: "建设单位1",
    //    sgdw: "施工单位1",
    //    cblx: "超标类型1",
   
    var convertData = function (data) {
        var res = [];
        for (var i = 0; i < data.length; i++) {
            var geoCoord = geoCoordMap[data[i].name];
            if (geoCoord) {
                res.push({
                    name: data[i].name,
                    value: geoCoord.concat(data[i].value)
                });
            }
        }
        return res;
    };


    var AREA_LOCATION_CONFIG = [["龙门", "114.1094970703125, 23.713696173823276, 120"], ['博罗', '114.21112060546875,23.324602156920964, 120'], ['惠东县', '114.9774169921875, 23.1959117878095, 120'],
     ['仲恺', '114.33403015136719,23.085416821274503, 120'], ['惠阳', '114.37110900878906,22.92804166565176, 120'], ['惠城', '114.52835083007812,23.24639340521435  , 120'],
     ['大亚湾', '114.57435607910156,22.818592860572195 , 120']];
    mapItemCountArray = new Array();
    var mapItemInfoArray = new Array();
    for (var i = 0; i < AREA_LOCATION_CONFIG.length; i++) {
        var areaname;
        var areaLocation;
        for (var j = 0; j < AREA_LOCATION_CONFIG[i].length; j++) {
            if (j == 0) {
                areaname = AREA_LOCATION_CONFIG[i][j];
            }
            if (j == 1) {
                areaLocation = AREA_LOCATION_CONFIG[i][j];
            }
        }
        $.ajax({
            type: "get",
            url: "GetItemByAreaId",
            dataType: "json",
            data: { "areaName": areaname, "areaLocation": areaLocation },
            async: false,
            success: function (result) {
                mapItemCountArray[i] = result;
                if (mapItemCountArray.length === 7) {
                    loadMap('/Content/javascripts/Echarts/huizhou.json', 'huizhou', mapItemCountArray);
                }
            }
        });
    }
    var timeFn = null;
    //单击切换到省级地图，当mapCode有值,说明可以切换到下级地图
    myChart.on('click', function (params) {
        clearTimeout(timeFn);
        //由于单击事件和双击事件冲突，故单击的响应事件延迟250毫秒执行
        timeFn = setTimeout(function () {
            var name = params.name; //地区name
            // 点击事件获取项目信息并且瞄点

  
            var mapCode = ququ[name]; //地区的json数据地址
            myChart.clear()
            if (!mapCode) {
                if (params.seriesType === "scatter") {
                    //alert("跳转到详情页，参数是：" + JSON.stringify(params.data));
                    //alert("这是项目ciaid,给光源使用：" + params.data.ciaId);
                    debugger
                    if (params.data.ciaId > 0) {
                        debugger
                        window.location.href = "/Project/GetItemInfo/" + params.data.ciaId;
                    }
                    
                } else {
                    console.log('无法加载地图')

                }
                return
            }
            console.log(params);
            console.log('你点击了一下');
            $.ajax({
                type: "get",
                url: "GetItemInfoLocation",
                dataType: "json",
                data: { "areaName": name},
                async: false,
                success: function (result) {
                    mapItemInfoArray = result;
                    building = mapItemInfoArray; //在建项目
                    //warming = mapItemInfoArray; //预警项目
                    loadMap(mapCode, name);
                }
            });
        }, 250);
    });


    // 绑定双击事件，返回惠州地图
    myChart.on('dblclick', function (params) {
        //当双击事件发生时，清除单击事件，仅响应双击事件
        clearTimeout(timeFn);
        myChart.clear();   //清楚返回时上个地图的信息
        //返回惠州地图
        console.log('你双击了')
        loadMap('/Content/javascripts/Echarts/huizhou.json', 'huizhou', mapItemCountArray);

    });

    /**
     获取对应的json地图数据，然后向echarts注册该区域的地图，最后加载地图信息
     @params {String} mapCode:json数据的地址
     @params {String} name: 地图名称
     */
    function loadMap(mapCode, name, warmingTotal) {
        $.get(mapCode, function (data) {
            echarts.registerMap(name, data);
            //惠州整体数据
            if (name == "huizhou") {
                var option = {
                    geo: {
                        map: name,
                        show: true,
                        roam: "move",
                        label: {
                            normal: {
                                show: true,

                                textStyle: {
                                    color: '#F7F7F7',
                                    fontSize: 14 * rate
                                }
                            },
                            emphasis: {
                                show: true,
                                color: 'yellow'
                            }
                        },
                        itemStyle: {
                            normal: {
                                areaColor: '#031525',
                                borderColor: '#1773c3',
                                shadowColor: '#031525',
                                shadowBlur: 20
                            },
                            emphasis: {
                                areaColor: '#051217',
                            },
                        },
                        zoom: 1.2,
                    },
                    series: [
                        //渲染地图
                        {
                            type: 'map',
                            coordinateSystem: 'geo',
                            zoom: 1.2,
                        },
                        {
                            name: '总预警',
                            type: 'scatter',
                            coordinateSystem: 'geo',//该系列使用的坐标系
                            data: warmingTotal,
                            symbol: 'image://data:image/png;base64,',
                            symbolSize: [60 * rate,35 * rate],
                            label: {
                                show: true,
                                formatter: function (params) {
                                    return '{a|' + params.data.total + '}'
                                },
                                rich: {
                                    a: {
                                        color: 'white',
                                        padding: [5 * rate, 0, 0, 10 * rate],
                                        fontSize:13 * rate

                                    }
                                }


                            }
                        },


                    ]
                };
            } else {
                var option = {
                    title: {
                        show: true,
                        x: 'right',
                        y: 'bottom',
                        textStyle: {
                            color: '#fff',
                            fontSize:13*rate
                        },
                        text: name,
                        padding: [80*rate, 40*rate]
                    },
                    //图例
                    legend: {
                        orient: 'vertical',
                        id: 1,
                        y: 'bottom',
                        x: 'right',
                        itemWidth: 15 * rate,
                        padding: [rate * 20, rate * 35],
                        data: [{
                            name: '在建项目',
                            icon: 'image://data:image/png;base64,'
                        },
                            {
                                name: '预警项目',
                                icon: 'image://data:image/png;base64,'
                            },

                        ],
                        textStyle: {
                            color: '#fff',
                            fontSize:13*rate
                        }
                    },
                    tooltip: {
                        show: true,
                        formatter: function (params) {
                            return "";
                        },
                    },
                    geo: {
                        map: name,
                        show: true,
                        roam: true,
                        label: {
                            normal: {
                                show: true,
                                textStyle: {
                                    color: '#F7F7F7',
                                    fontSize: 13 * rate
                                },
                            },
                            emphasis: {
                                show: true,
                                color: 'yellow'
                            }
                        },
                        itemStyle: {
                            normal: {
                                areaColor: '#031525',
                                borderColor: '#1773c3',
                                shadowColor: '#031525',
                                shadowBlur: 20
                            },
                            emphasis: {
                                areaColor: '#051217',
                            },
                        },
                        zoom: 1.2,
                    },
                    series: [{
                        name: 'MAP',
                        type: 'map',
                        zoom: 1.2,


                    },
                        // 标志
                        {
                            name: '在建项目',
                            type: 'scatter',
                            coordinateSystem: 'geo',
                            data: building,
                            symbol: 'image://data:image/png;base64,',
                            symbolSize: [35 * rate,40*rate],
                            //不能去
                            label: {
                                show: false, formatter: function (params) {
                                    return ''
                                }
                            },
                            tooltip: {
                                backgroundColor: "#17334D",
                                formatter: function (data) {
                                    var data = data.data;
                                    var str = "<div class='tips_map'>\
										<div style='border-bottom:2px solid #0D1923;padding-bottom:0.1rem'>\
											<span>" + data.name + "</span>\
											<span class='status1'>" + data.status + "</span>\
										</div>\
										<div>建设单位：" + data.jsdw + "</div>\
										<div>施工单位：" + data.sgdw + "</div>\
										<span class='err1'>" + data.cblx + "</span>\
										</div>";
                                    return str;
                                }
                            },

                        },
                        {
                            name: '预警项目',
                            type: 'scatter',
                            coordinateSystem: 'geo',//该系列使用的坐标系
                            data: warming,
                            symbol: 'image://data:image/png;base64,',
                            symbolSize: 35 * rate,
                            label: {show: false},
                            tooltip: {
                                backgroundColor: "#17334D",
                                formatter: function (data) {
                                    var data = data.data;
                                    var str = "<div class='tips_map'>\
										<div style='border-bottom:2px solid #0D1923;padding-bottom:0.1rem'>\
											<span>" + data.name + "</span>\
											<span class='status1'>" + data.status + "</span>\
										</div>\
										<div>建设单位：" + data.jsdw + "</div>\
										<div>施工单位：" + data.sgdw + "</div>\
										<span class='err1'>" + data.cblx + "</span>\
										</div>";
                                    return str;
                                }
                            },
                        },

                    ]
                };
            }
            myChart.setOption(option);
            // window.onresize=function () {
            // 	myChart.resize()
            // }
            window.addEventListener('resize', function () {
                console.log(rate)
                myChart.resize();

            });
            // curMap = {
            //     mapCode: mapCode,
            //     mapName: name
            // };

        });
    }
})